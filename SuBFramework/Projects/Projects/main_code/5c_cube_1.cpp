...
public:
  ...
  virtual void render(double currentTime) {
    ...
    model = glm::translate(model, glm::vec3(sinf(currentTime * 1.3f),
                                                               cosf(currentTime * 1.72f),
                                                               sinf(currentTime * 1.19f) * cosf(currentTime * 1.33f) - 5.0f));

    model = glm::rotate(model, (float)currentTime, glm::vec3(-1.0f, 1.0f, 0.0f));
    ...
    glUniformMatrix4fv(glGetUniformLocation(m_program.getProgramID(), "projection"), 1, GL_FALSE, glm::value_ptr(projection));
