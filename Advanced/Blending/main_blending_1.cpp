...
int main() {
    ...
    Program blendingProgram("Advanced/Blending/veg_vertex.glsl", "Advanced/Blending/veg_fragment.glsl");
    if(blendingProgram.hasError()) {
        std::cout << blendingProgram.getErrorMessage() << std::endl;
        return -1;
    }

    // TEXTURE LOADING -------------------------------------------------------

    unsigned int textureDiffuse, textureSpecular, vegTexture;
    ...
    glBindVertexArray(vegVAO);
    glBindBuffer(GL_ARRAY_BUFFER, vegVAO);
