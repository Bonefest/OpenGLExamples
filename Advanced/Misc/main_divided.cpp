...
int main() {
    ...
    // SHADER PREPARING ------------------------------------------------------
    Program program("Lights/Shaders/lc_vertex.glsl", "Advanced/Misc/left_fragment.glsl");
    if(program.hasError()) {
        std::cout << program.getErrorMessage() << std::endl;
        return -1;
    }

    Program rprogram("Lights/Shaders/lc_vertex.glsl", "Advanced/Misc/right_fragment.glsl");
    if(rprogram.hasError()) {
        std::cout << rprogram.getErrorMessage() << std::endl;
        return -1;
    }
    ...
    glUniform1f(glGetUniformLocation(program.getProgramID(), "screenWidth"), WIDTH);

    glUseProgram(rprogram.getProgramID());
    glUniform1f(glGetUniformLocation(rprogram.getProgramID(), "screenWidth"), WIDTH);
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        glUseProgram(rprogram.getProgramID());
        glUniformMatrix4fv(glGetUniformLocation(rprogram.getProgramID(), "view"), 1, GL_FALSE, glm::value_ptr(view));
        glUniformMatrix4fv(glGetUniformLocation(rprogram.getProgramID(), "proj"), 1, GL_FALSE, glm::value_ptr(proj));
        ...
        for(auto cubeData : cubesData) {
            ...
            glUniformMatrix4fv(glGetUniformLocation(rprogram.getProgramID(), "model"), 1, GL_FALSE, glm::value_ptr(model));
